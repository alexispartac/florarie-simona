    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
        ...
        <div className="hidden md:flex items-center gap-2">
          <span className="text-sm font-medium text-gray-700">{t('shop.filterBy')}</span>
          ...
          {(searchQuery || selectedCategories.length > 0) && (
            ...
            >
              <X className="h-4 w-4" /> {t('shop.clearFilters')}
            </button>
          )}
        </div>
      </div>
      ...
      {showFilters && (
        <div className="md:hidden mb-6 p-4 bg-white rounded-lg shadow">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-medium">{t('shop.mobileFilters')}</h3>
            <button
              onClick={onToggleFilters}
          ...
          <div className="space-y-4">
            <div>
              <h4 className="text-sm font-medium text-gray-700 mb-2">{t('shop.categories')}</h4>
