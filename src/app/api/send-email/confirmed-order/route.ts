...
export async function POST(request: Request) {
  try {
    ...
    // Send the email
    await sendOrderConfirmedEmail(orderToSend);
    ...
    console.error('Error in send-email/confirmed-order:', error);
    return NextResponse.json(
      { 
        success: false,
        error: error instanceof Error ? error.message : 'Failed to send order confirmed email' 
      },
      { status: 500 }
    );
  }
