...
export async function POST(req: NextRequest) {
  ...
  try {
    const { clientEmail, clientName, order } : { clientEmail: string; clientName: string; order: OrderProps } = await req.json();
        
    // Verifică dacă toate câmpurile necesare sunt prezente
    if (!clientEmail || !clientName || !order) {
    ...
    if (!order.clientEmail || !order.clientName || !order.orderNumber || !order.orderDate || !order.status || !order.products || order.products.length === 0 || !order.totalPrice || !order.paymentMethod || !order.userId || !order.id || !order.clientPhone || !order.clientAddress) {
      return NextResponse.json(
